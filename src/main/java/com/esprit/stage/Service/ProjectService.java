...
public class ProjectService {
    ...
    public Project updateProject(Long id, Project updatedProject) {
        logger.info("✏️ Mise à jour du projet avec ID: {}", id);
    ...
    public void deleteProject(Long id) {
        logger.info("🗑️ Suppression du projet avec ID: {}", id);
    ...
    public float updateProjectProgress(Long projectId) {
        ...
        if (totalTasks == 0) {
            logger.warn("⚠️ Aucune tâche trouvée pour le projet ID: {}. Progression mise à 0%.", projectId);
            return 0.0f;
        }
                ...
                if (daysRemaining <= 3 && daysRemaining >= 0) {
                    ...
                    for (User intern : interns) {
                        if (intern.getEmail() != null && !intern.getEmail().isEmpty()) {
                            ...
                            logger.warn("⚠️ L'utilisateur '{}' (INTERN) n'a pas d'email associé.", intern.getUsername());
                        }
                    }
                }
            }
    ...
    private void sendDeadlineEmail(User intern, Project project, long daysRemaining) {
        ...
        String htmlContent = "<!DOCTYPE html>" +
                ...
                "<a class='btn' href='http://localhost:4200/list-task/" + project.getId() + "'>see project</a>" +
   ...
   // @Scheduled(cron = "*/30 * * * * ?") // Exécute toutes les 30 secondes

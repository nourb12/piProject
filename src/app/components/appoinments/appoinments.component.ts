        ...
        if (result) {
          if (result.delete) {
            ...
            if (result.isEditMode) {
              ...
              this.interviewService.addInterview(result.interview, result.interview.studentId, result.interview.hrId,result.interview.offreId).subscribe(newEvent => {
    ...
    openModal(): void {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          ...
          const { interview, studentId, hrId , offreId} = result;
    
          if (interview && studentId && hrId && offreId) {
            this.interviewService.addInterview(interview, studentId, hrId , offreId).subscribe(newEvent => {
              this.Interviews.push(this.mapToCalendarEvent(newEvent));
              this.refresh.next(); 
            });
          } else {

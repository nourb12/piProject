...
export class QuizService {
  ...
  private baseUrl = 'http://localhost:8093/quiz/api/quiz'; // Assurez-vous que cette URL est correcte
  ...
  saveResult(result: any, quizId: number): Observable<any> {
    return this.http.post(`http://localhost:8093/quiz/api/result/create/${quizId}`, result);
  }
  ...
  getQuizzesBySpecialite(specialite: string): Observable<any[]> {
    return this.http.get<any[]>(`http://localhost:8093/quiz/api/quiz/bySpecialite/${specialite}`);
  }
  
  getResultsBySpecialty(): Observable<any> {
    return this.http.get<any>('http://localhost:8093/quiz/api/quiz/stats/results-by-specialty');
  }
